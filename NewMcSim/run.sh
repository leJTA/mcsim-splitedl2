...
cat output/$str.out | grep "L2\$I\[000\] : RD" | awk -F "[()]" '{print $4}' | awk -F "," '{print $2 "," $1 ","}' | xargs -I {} -t echo -n {} >> output/results.csv #L2 RD
cat output/$str.out | grep "L2\$D\[000\] : RD" | awk -F "[()]" '{print $4}' | awk -F "," '{print $2 "," $1 ","}' | xargs -I {} -t echo -n {} >> output/results.csv #L2 RD
cat output/$str.out | grep "L2\$D\[000\] : WR" | awk -F "[()]" '{print $4}' | awk -F "," '{print $2 "," $1}' | xargs -I {} -t echo -n {} >> output/results.csv #L2 WR
printf "\n" >> output/results.csv
